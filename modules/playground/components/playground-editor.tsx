...
export const PlaygroundEditor = ({
  ...
  const suggestionAcceptedRef = useRef(false)
  const suggestionTimeoutRef = useRef<NodeJS.Timeout | null>(null)
  const tabCommandRef = useRef<number | null>(null) // <-- number now
  ...
  const createInlineCompletionProvider = useCallback(
    (monaco: Monaco) => ({
      provideInlineCompletions: async (model: any, position: any, context: any, token: any) => {
        if (isAcceptingSuggestionRef.current || suggestionAcceptedRef.current) return { items: [] }
  ...
  const handleEditorDidMount = (editor: any, monaco: Monaco) => {
    ...
    editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.Space, () => onTriggerSuggestion("completion", editor))
    ...
    editor.onDidChangeCursorPosition((e: any) => {
      ...
      if (currentSuggestionRef.current && !suggestionAcceptedRef.current) {
        const suggestionPos = currentSuggestionRef.current.position
        if (newPosition.lineNumber !== suggestionPos.line || newPosition.column < suggestionPos.column || newPosition.column > suggestionPos.column + 10) {
          clearCurrentSuggestion()
          onRejectSuggestion(editor)
        }
      }
      if (!currentSuggestionRef.current && !suggestionLoading) {
        ...
        suggestionTimeoutRef.current = setTimeout(() => onTriggerSuggestion("completion", editor), 300)
      }
    })
  ...
  useEffect(() => updateEditorLanguage(), [activeFile])
  ...
  useEffect(() => {
    return () => {
      ...
      tabCommandRef.current = null // <-- removed dispose()
    }
  }, [])
